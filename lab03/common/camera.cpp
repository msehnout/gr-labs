...
void Camera::update()
{
    ...
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
        position -= glm::vec3(0, 0, 1) * deltaTime * speed;
    }
    // Move backward
    ...
    viewMatrix = lookAt(
		...
		//position - vec3(0, 0, 5), 
		vec3(0, 0, -100), // and looks at the origin
    ...
    //*/
    // and cursor position
    horizontalAngle += deltaTime * mouseSpeed * (xPos-(width/2.0));
    verticalAngle += deltaTime * mouseSpeed * (yPos - (height / 2.0));
    ...
    viewMatrix = lookAt(
        position, // to je jasny asi
		position + direction, // direction je jednotkovy vektor ve smeru pohledu
		up // netusim
    );
    //*/
